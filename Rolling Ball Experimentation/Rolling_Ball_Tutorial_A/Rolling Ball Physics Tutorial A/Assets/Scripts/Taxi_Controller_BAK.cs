...
public class Taxi_Controller_BAK : MonoBehaviour 
{
    ...
    [SerializeField]
    private bool legacyControl = false;
  ...
  private void Start()
    ...
    void Update()
    {
        ...
        if (legacyControl)
        {
            moveSideways = Input.GetAxis("Vertical");
            moveForward = Input.GetAxis("Horizontal");

            movement = new Vector3(moveSideways * thrustMultiplier, 0.0f, moveForward * thrustMultiplier * -1.0f);
            rb.AddForce(movement);
      
            UseGas(gasUseRateUpThrust);
               
            if (Input.GetAxis("Jump") > 0 && hasControl)
            {
                if (isGrounded)
                {
                    upwardThrust = 5.0f;
                    movement = new Vector3(0.0f, upwardThrust * thrustMultiplier, 0.0f);
                    rb.AddForce(movement);
                    UseGas(gasUseRateUpThrust);
                }
            }
        ...
        else
        {
            ...
            if (hasControl)
            {
                ...
                movement = Quaternion.AngleAxis(angle + 90.0f,
                    Vector3.up * moveForward * thrust * thrustMultiplier)
      ...
      else
      {
        ...
        // Vertical upward thrust, uses A button on XBox controller
        if (Input.GetAxis("Fire1") >= 0.1f && hasControl)
        {
            ...
            rb.AddForce(movement);
            UseGas(gasUseRateUpThrust);
            FireEngines(enginesUp, upJetScale);
        }
        if (Input.GetAxis("Fire1") < 0.15f)
        {
            StopEngines(enginesUp);
        }


        //Vertical downwart thrust, uses Y button on XBox controller
        // !!! Rename "Jump" to something mroe appropriate later
        if (Input.GetAxis("Jump") >= 0.01f && hasControl)
        {
            upwardThrust = -1.0f;
            movement = new Vector3(0.0f, upwardThrust * thrustMultiplier, 0.0f);
            rb.AddForce(movement);
            UseGas(gasUseRateDownThrust);
            FireEngines(enginesDown, downJetScale);
        }
  ...
  */
  ...
  void FireEngines( GameObject[] whichengines, float scale ) // ADD SECOND VAR, a scale factor, which gets passed above by the various thrust routines
  {
    ...
    //int newSize = 30;
    //Array.Resize( ref originalScale, newSize ); // Kludge. 
    ...
    //  originalScale[i] = whichengines[ i ].transform.localScale;
      ...
      whichengines[ i ].transform.localScale = new Vector3 ( curScale.x * 1.0f, scale, curScale.z * 1.0f); // !!!!! Update the 1.0f in .y to the value passed to this method
        }
    }
    ...
    void LoseControl()
    {
        ...
        Vector3 StartMovement = new Vector3(((UnityEngine.Random.value - .5f) * 10), 0.0f, (Math.Abs(UnityEngine.Random.value) * -40));
